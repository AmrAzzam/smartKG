...
public class LinkedDataFragmentGraph extends GraphBase {
    ...
    public LinkedDataFragmentGraph(String dataSource) {
        super();
        //
        //  System.out.println("======= Start of ldfClient ======= ");
        //  System.out.println("DataSource" + dataSource);
        this.ldfClient = new LinkedDataFragmentsClient(dataSource);
        //  System.out.println("======= End of ldfClient ======= ");
        //
    ...
    /**
     * Answer the capabilities of this graph; the default is an AllCapabilities
     * object (the same one each time, not that it matters - Capabilities should
     * be immutable).
     */
    @Override
    public Capabilities getCapabilities() {
        if (capabilities == null) {
            capabilities = new LinkedDataFragmentGraphCapabilities();
        }
        return capabilities;
    }
    ...
    protected ExtendedIterator<Triple> graphBaseFind(Triple m) {
        try {
         ...
         //   if (m.getPredicate().toString().contains("caption")) {
           //     x++;
                //System.out.println("LinkedDataFragmentsClient" + x + " " + m);
            //}

           // Stopwatch stopwatch = Stopwatch.createStarted();

            //System.out.println("graphBaseFind ===>" + m);
            LinkedDataFragment ldf = ldfClient.getFragment(ldfClient.getBaseFragment(), m);

            
          //  if (m.getPredicate().toString().contains("caption")) {
           // System.out.println("Time elapsed for Results is " + stopwatch.elapsed(TimeUnit.MICROSECONDS ));
            //}
